...
LensDNS dnsproxy 是基于 [AdguardTeam/dnsproxy](https://github.com/AdguardTeam/dnsproxy) 的加固分支，面向代理感知与负载均衡后置部署：在 LB 之后保留真实客户端身份，并强化现代 TCP/TLS 运维语义（含 TCP/DoT 上的 PPv2、连接生命周期与相关可调项）。
...
**Client → LB** 这一段不承载 PPv2；启用 PPv2 时，由负载均衡在 **LB → dnsproxy** 链路上注入，后端在收紧的 `TrustedProxies` 策略下恢复原始客户端地址。详见 [docs/zh/ppv2.md](docs/zh/ppv2.md)。

## 核心能力

- **Proxy Protocol v2（PPv2）**：DNS-over-TCP 与 DoT 入口（严格模式、PPv2 读取超时可调）
- **真实客户端 IP**：在 L4/L7 负载均衡后，与适当的 **可信代理** 配置及 PPv2 联用（见 [docs/zh/trusted-proxies.md](docs/zh/trusted-proxies.md)）
- **TCP 分帧健壮性**：RFC 1035 两字节长度前缀始终完整读取（应对 TCP 短读）
- **连接生命周期**：高并发下 TCP 关闭阶段 RST 相关边界行为的缓解
- **DoT**：独立 TLS 超时语义、TCP keep-alive 默认值、支持平台上的 **TCP Fast Open（TFO）**
- **TLS 会话恢复**：DoT/HTTPS 服务端与 DoH/DoQ/DoT 上游客户端侧
- **`max-go-routines` 默认 `32`**：面向常见 LB 后置场景的基线
- **Go module** `github.com/LensDNS/dnsproxy`：与上游在构建/依赖/分发上明确区分
...
从 [GitHub Releases](https://github.com/LensDNS/dnsproxy/releases) 下载。自动化部署建议优先于本地临时构建。

### 从源码构建

需要 Go **1.26+**，执行 `make build`。详见下方上游 README 的 **How to build**。

## 为什么需要 LensDNS dnsproxy

生产环境中 DNS 常位于 HAProxy、Nginx、云负载均衡或其他代理之后。若缺少将**原始客户端**传到后端的传输层机制，**限流、滥用归因、GeoDNS、安全观测**都会退化为只能识别负载均衡，而看不到终端用户。

本分支在 **LB → dnsproxy** 一侧为 **TCP/DoT** 提供 **PPv2**，在配置得当的前提下恢复客户端地址语义，并把 **信任边界** 写清楚：`TrustedProxies`、网络隔离、禁止在公网边界接受由客户端操控的、不安全的 PPv2。同时对 **TCP/TLS 生命周期** 做了与该类部署相匹配的加固。

## 文档

从 **[中文文档索引](docs/zh/README.md)** 进入（正文为中文）；英文版见 [docs/README.md](docs/README.md)。

- [Proxy Protocol v2（PPv2）](docs/zh/ppv2.md)
- [可信代理与客户端地址信任](docs/zh/trusted-proxies.md)
- [TCP/TLS 行为](docs/zh/tcp-tls-behavior.md)
- [测试与网络依赖用例](docs/zh/testing.md)
